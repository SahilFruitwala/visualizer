...
type Phase = "cache-miss" | "to-resolver" | "to-tld" | "to-auth" | "auth-reply" | "tld-reply" | "resolver-reply" | "cached";
...
function build(): Step[] {
  return [
    ...
    },
    {
      ...
      caption: "TLD returns NS records → resolver asks authoritative server for the A record.",
    },
    {
      phase: "auth-reply",
      activeSide: "server",
    ...
    },
    {
      ...
      chapter: "IP returned",
      caption: "Browser receives IP — can now open TCP connection. ✓",
    },
    {
      phase: "cached",
      activeSide: "both",
...
const DNS_STEPS = withCodeLines(build(), (s) => {
  ...
  if (s.phase === "to-resolver" || s.phase === "to-tld" || s.phase === "to-auth") return [2, 3, 4];
